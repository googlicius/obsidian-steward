...
export class CommandInputService {
  ...
  private createSelectionMarker(filePath: string): string {
    ...
    return `{{stw-source type:selected,path:${filePath},from:${cursorFrom.line},to:${cursorTo.line},selection:${new MarkdownUtil(selection).escape(true).getText()}}}`;
  }
  ...
  public async addSelectionToConversation(target: 'inline' | 'chat' = 'inline'): Promise<void> {
    try {
      ...
      if (target === 'chat') {
        ...
        const chatLeaf = await this.plugin.getStewardLeaf();
