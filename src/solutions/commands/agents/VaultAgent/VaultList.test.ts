...
describe('VaultList', () => {
  ...
  let mockAgent: jest.Mocked<VaultAgent>;
  ...
  beforeEach(() => {
    ...
    mockAgent = {
      app: mockPlugin.app,
    } as unknown as jest.Mocked<VaultAgent>;
  ...
  describe('executeListTool', () => {
    ...
    it('should list all images', async () => {
      ...
      // Pattern to match image files: .png, .jpg, .jpeg, .gif, .webp
      const result = await executeListTool(
        { folderPath: 'folder', filePattern: '\\.(png|jpg|jpeg|gif|webp)$', explanation: '' },
        null
      );
