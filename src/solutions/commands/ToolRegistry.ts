...
NOTE:
  ...
  [ToolName.SHOW_WIDGET]: {
    ...
    guidelines: [
      ...
      'For updatable HTML widgets, use project mode: files as [{ name, content }, ...] — split index.html, style.css, main.js, etc. instead of one inline HTML blob. Link them from index.html (<link href="style.css">, <script src="main.js">); they are bundled into one document at render. Files live in a vault project folder and hot-reload on edit.',
      'For a single HTML or SVG blob, pass code instead of files. Code must be self-contained with inline styles/scripts, no external CDN.',
      'Use {{widget-asset:key}} in HTML for manifest assets; use vault:path in assets array for images or markdown to bundle at render time.',
      'After rendering a project widget, do NOT call show_widget again for updates. Use EDIT and CONTENT_READING on the projectPath returned in the tool result.',
